...
#define POWER_SENSE PB1 // pin that's connected to ACC

#define SCHEDULLED_TASK_BL_AUDIO_ON_OFF 0x01
...
volatile uint8_t schedulled_task = 0;
...
ISR(INT0_vect)
{
    if (PINB & _BV(POWER_SENSE)){ // power is on
        // ipod_enable_port();
        bl_audio_init();
        first_play = 1;
    } else {
        // ipod_disable_port(); // save some power
        // schedulled_task |= (1 << SCHEDULLED_TASK_BL_AUDIO_ON_OFF);
    }
}
